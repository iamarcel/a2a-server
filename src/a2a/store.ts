...
export class FileStore implements TaskStore {
  ...
  private async ensureDirectoryExists(): Promise<void> {
    try {
      ...
      throw A2AError.internalError(
        `Failed to create directory ${this.baseDir}: ${String(error)}`,
        error,
      );
    }
  }
  ...
  private isHistoryFileContent(
    ...
    return (
      ...
      content !== null &&
      Array.isArray(
        (content as { messageHistory: unknown[] }).messageHistory,
      ) &&
      // Optional: Add deeper validation of message structure if needed
      (content as { messageHistory: unknown[] }).messageHistory.every(
        (msg: unknown) =>
          typeof msg === "object" &&
          msg &&
          (msg as schema.Message).role &&
          (msg as schema.Message).parts,
      )
    );
  }
  ...
  private async writeJsonFile(filePath: string, data: unknown): Promise<void> {
    try {
      ...
      throw A2AError.internalError(
        `Failed to write file ${filePath}: ${String(error)}`,
        error,
      );
    }
  }
